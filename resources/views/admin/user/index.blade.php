...
<div class="card border-0 shadow mb-4" style="border-radius: 15px;">
    <div class="card-body">
            ...
            <table class="table table-hover align-items-center">
                ...
                <tbody>
                    @foreach ($dataUser as $row)
                        <tr>
                            <td class="text-muted fw-bold">{{ ($dataUser->currentPage() - 1) * $dataUser->perPage() + $loop->iteration }}</td>
                            <td>
                                @if($row->avatar)
                                    <img src="{{ asset('avatar/' . $row->avatar) }}" class="rounded-circle shadow-sm" style="width: 45px; height: 45px; object-fit: cover; border: 2px solid #D35400;">
                                @else
                            ...
                            <td><span class="fw-bold" style="color: #5A3E2B;">{{ $row->name }}</span></td>
                            <td>{{ $row->email }}</td>
                            <td>
                                @php
                                ...
                                @endphp
                                <span class="badge-role" style="{{ $roleStyle }}">
                                    {{ $row->role }}
                                </span>
                            </td>
                            <td class="text-center">
                                <div class="btn-group">
                                    <a href="{{ route('user.edit', $row->id) }}" class="btn btn-sm me-2 shadow-sm"
                                       style="background: #FFF4E6; color: #5A3E2B; border: 1px solid #5A3E2B; border-radius: 8px;">
                                        Edit
                                    </a>
                                    <a href="{{ route('user.destroy', $row->id) }}" class="btn btn-sm text-white shadow-sm"
        ...
        {{-- Bagian Navigasi yang sudah bersih dari teks --}}
