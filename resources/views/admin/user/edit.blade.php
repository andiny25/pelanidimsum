...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
    <div class="d-block mb-4 mb-md-0">
        ...
        <p class="mb-0" style="color: #8B5E3C;">Form Perubahan Data Pengguna: <strong>{{ $data_user->name }}</strong></p>
    </div>
        ...
        <div class="row">
            ...
            <div class="col-md-12 mb-4">
                ...
                <div class="d-flex align-items-center gap-3">
                    {{-- @if($data_user->avatar)
                        <img src="{{ asset('storage/avatars/' . $data_user->avatar) }}" class="avatar-preview" alt="Current Avatar">
                    @else
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($data_user->name) }}&background=FFF4E6&color=D35400" class="avatar-preview" alt="Default Avatar">
                    @endif --}}
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="text" name="name" id="name" class="form-control border-light shadow-sm"
                       value="{{ $data_user->name }}" required style="border-radius: 8px; padding: 12px;">
            </div>

            <div class="col-md-6 mb-3">
                <label for="email">Email</label>
                <input type="email" name="email" id="email" class="form-control border-light shadow-sm"
                       value="{{ $data_user->email }}" required style="border-radius: 8px; padding: 12px;">
            </div>
            ...
            <div class="col-md-6 mb-3">
                ...
                <select class="form-select border-light shadow-sm" id="role" name="role" required style="border-radius: 8px; padding: 12px;">
                    <option value="Super Admin" {{ $data_user->role == 'Super Admin' ? 'selected' : '' }}>Super Admin</option>
                    <option value="Administrator" {{ $data_user->role == 'Administrator' ? 'selected' : '' }}>Administrator</option>
                    <option value="Pelanggan" {{ $data_user->role == 'Pelanggan' ? 'selected' : '' }}>Pelanggan</option>
                    <option value="Mitra" {{ $data_user->role == 'Mitra' ? 'selected' : '' }}>Mitra</option>
                </select>
            </div>
        </div>
